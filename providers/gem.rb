action :install do
  gem_exec = "source /usr/local/opt/chruby/share/chruby/chruby.sh && RUBIES=(/usr/local/var/rubies/#{new_resource.ruby_version}) && chruby #{new_resource.ruby_version} && /usr/local/var/rubies/ruby-#{ruby}/bin/gem"
